...
class StreamingBridge {
  static const MethodChannel _channel = MethodChannel(
    'com.playnite.companion/streaming_bridge',
  );

  Future<List<HostInfo>> discoverHosts() async {
    final List<dynamic> result =
        await _channel.invokeMethod<List<dynamic>>('discoverHosts') ?? [];
    return result
        .map((entry) => HostInfo.fromMap((entry as Map).cast<dynamic, dynamic>()))
        .toList();
  }
  ...
  Future<bool> pairWithPin({
    ...
    final bool? paired = await _channel.invokeMethod<bool>('pairWithPin', {
      'hostId': hostId,
      'pin': pin,
    });
    return paired ?? false;
  ...
  Future<bool> startStream({
    ...
    final bool? started = await _channel.invokeMethod<bool>('startStream', {
      'hostId': hostId,
      'width': width,
      'height': height,
      'fps': fps,
